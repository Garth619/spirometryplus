...
class OrderManager extends OrderManagerAbstract
{
    ...
    private function formatOrder($orderObj)
    {
        ...
        $order->processed_at = Helper::formatDate($orderDate);
        $order->cancelled_at = $orderObj->get_status() == 'cancelled' ? Helper::formatDate($orderDateModified) : '';
        ...
        $orderCustomer->signed_up_at = Helper::formatDate($orderDate);
        ...
        if ($orderObj->get_user()) {
            ...
            if ($contact) {
                if ($contact->email === $orderCustomer->email) {
                    $overrideFields = array('first_name', 'last_name', 'address1', 'address2', 'city', 'province',
                        'zip', 'country');
